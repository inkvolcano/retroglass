...
// Passthrough vertex for custom ESSL 3.00 fragments (ESSL 1.00 fragments should ship
// their own vertex or reuse the attribute names below with 1.00 syntax).
const std::string ShaderManager::customDefaultVertexV3 =
    "#version 300 es\n"
    "in vec4 vPosition;\n"
    "in vec2 vCoordinate;\n"
    "out vec2 coords;\n"
...
/** Builds a chain from params produced by GLRetroView for ShaderConfig.Custom. On any
 *  malformed input it degrades to the default single-pass chain instead of crashing. */
ShaderManager::Chain ShaderManager::buildCustomChain(
    const std::unordered_map<std::string, std::string>& params
) {
    auto get = [&params](const std::string& key, const std::string& fallback) {
    ...
    int passCount = 0;
    try {
        passCount = std::stoi(get("PASSES", "0"));
    } catch (const std::exception&) {
        passCount = 0;
    }

    Chain result;
    result.linearTexture = isTrue(get("LINEAR_TEXTURE", "1"));

    for (int i = 0; i < passCount; ++i) {
        std::string prefix = "PASS" + std::to_string(i) + "_";
        auto fragmentIt = params.find(prefix + "FRAGMENT");
        if (fragmentIt == params.end()) {
            passCount = 0;
            break;
        }

        Pass pass;
        pass.vertex = get(prefix + "VERTEX", customDefaultVertexV3);
        pass.fragment = fragmentIt->second;
        pass.linear = isTrue(get(prefix + "LINEAR", "1"));
        pass.float16 = isTrue(get(prefix + "FLOAT16", "0"));
        try {
            pass.scale = std::stof(get(prefix + "SCALE", "1.0"));
        } catch (const std::exception&) {
            pass.scale = 1.0F;
        }
    ...
    if (passCount <= 0 || result.passes.empty()) {
        return { { { defaultShaderVertex, defaultShaderFragment, true, 1.0 } }, true };
    }
