...
public:
    struct Pass {
        ...
        bool linear;
        float scale;
        // RGBA16F framebuffer for signed/HDR intermediates (CNN feature maps). Falls
        // back to RGBA8 at runtime when half-float attachments are unsupported.
        bool float16 = false;
    ...
    enum class Type {
        ...
        // Arbitrary multi-pass chain: the GLSL sources travel inside Config.params
        // (PASSES, PASSn_FRAGMENT, PASSn_VERTEX?, PASSn_SCALE, PASSn_LINEAR, PASSn_FLOAT16).
        SHADER_CUSTOM = 7,
    };
...
private:
    static const std::string customDefaultVertexV3;
