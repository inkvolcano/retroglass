...
void Video::updateProgram() {
    ...
    // A custom chain that fails to compile/link must not take the app down: degrade to
    // the default shader instead (kept until the user picks another filter).
    if (requestedShaderConfig.type == ShaderManager::Type::SHADER_CUSTOM &&
        !buildShadersChain(shaders)) {
        LOGE("Custom shader chain failed; falling back to the default shader");
        shaders = ShaderManager::getShader(
            ShaderManager::Config { ShaderManager::Type::SHADER_DEFAULT, {} }
        );
        buildShadersChain(shaders);
    } else if (requestedShaderConfig.type != ShaderManager::Type::SHADER_CUSTOM) {
        if (!buildShadersChain(shaders)) {
...
/** Compiles and links every pass of [shaders]; false (with a clean state) on failure. */
bool Video::buildShadersChain(const ShaderManager::Chain& shaders) {
    shadersChain = {};
    ...
    for (const auto& item : shaders.passes) {
        ...
        if (shader.gTextureHandle == -1) {
            // ESSL 3.00 shaders can't name a sampler "texture" (builtin); custom passes
            // use "mainTexture" instead.
            shader.gTextureHandle = glGetUniformLocation(shader.gProgram, "mainTexture");
        }
        ...
        if (shader.gTextureSizeHandle == -1) {
            // Same story: "textureSize" is an ESSL 3.00 builtin function name.
            shader.gTextureSizeHandle = glGetUniformLocation(shader.gProgram, "sourceSize");
        }
