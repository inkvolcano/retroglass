...
std::unique_ptr<ES3Utils::Framebuffer> ES3Utils::createFramebuffer(
    ...
    // RGBA16F needs EXT_color_buffer_half_float to be renderable; the status check below
    // catches unsupported devices and we retry with plain RGBA8.
    glTexStorage2D(GL_TEXTURE_2D, 1, float16 ? GL_RGBA16F : GL_RGBA8, width, height);
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        if (float16) {
            // Half-float attachment unsupported here: degrade gracefully to RGBA8.
            LOGE("RGBA16F framebuffer incomplete, falling back to RGBA8");
            glBindTexture(GL_TEXTURE_2D, 0);
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
            glBindRenderbuffer(GL_RENDERBUFFER, 0);
            deleteFramebuffer(std::move(result));
            return createFramebuffer(width, height, linear, repeat, includeDepth, includeStencil, false);
        }
