...
class GLRetroView(
    ...
    private fun buildShader(config: ShaderConfig): GLRetroShader {
        return when (config) {
            ...
            is ShaderConfig.Custom -> {
                val params = mutableMapOf<String, String>()
                params["PASSES"] = toParam(config.passes.size)
                params["LINEAR_TEXTURE"] = toParam(config.linearTexture)
                config.passes.forEachIndexed { i, pass ->
                    params["PASS${i}_FRAGMENT"] = pass.fragment
                    pass.vertex?.let { params["PASS${i}_VERTEX"] = it }
                    params["PASS${i}_SCALE"] = toParam(pass.scale)
                    params["PASS${i}_LINEAR"] = toParam(pass.linear)
                    params["PASS${i}_FLOAT16"] = toParam(pass.float16)
                }
                GLRetroShader(LibretroDroid.SHADER_CUSTOM, params)
            }
        }
    }
